#!/bin/bash
docker exec -it mssql-container-test /opt/mssql-tools/bin/sqlcmd -S localhost -U SA -P "$(read -sp "Enter current SA password: "; echo "${REPLY}")" -Q "ALTER LOGIN SA WITH PASSWORD=\"$(read -sp "Enter new SA password: "; echo "${REPLY}")\""
...
time docker exec mssql-container-test sh -c "/opt/mssql-tools/bin/sqlcmd -S localhost -U amelia_user -PAdmin123! -C -i /tmp/sql-server-sakila-db/sql-server-sakila-insert-data.sql"
time docker exec mssql-container-test sh -c "/opt/mssql-tools/bin/sqlcmd -S localhost -U amelia_user -PAdmin123! -C -i /tmp/sql-server-sakila-db/sql-server-sakila-delete-data.sql"
time docker exec mssql-container-test sh -c "/opt/mssql-tools/bin/sqlcmd -S localhost -U amelia_user -PAdmin123! -C -i /tmp/sql-server-sakila-db/sql-server-sakila-drop-objects.sql"
